...
describe("automatic offer validity and monitoring", () => {
  it("warns in the last minute and expires only after the existing 390-second boundary", () => {
    expect(automaticOfferValidity(asOf, now + 329_999).status).toBe("fresh");
    expect(automaticOfferValidity(asOf, now + 330_000)).toEqual({
      status: "expiring", expiresAt: "2026-09-12T10:06:30.000Z", secondsRemaining: 60,
    });
    expect(automaticOfferValidity(asOf, now + 390_000).status).toBe("expiring");
    expect(automaticOfferValidity(asOf, now + 390_001).status).toBe("expired");
  });

  it("rejects missing, malformed and future references", () => {
    expect(automaticOfferValidity(undefined, now).status).toBe("unavailable");
    expect(automaticOfferValidity("bad", now).status).toBe("invalid");
    expect(automaticOfferValidity(asOf, now - 1).status).toBe("invalid");
  });
